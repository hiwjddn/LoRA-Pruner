...
def main():
    ...
    output_dir = f"{args.output_dir}/{args.seed}"

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())

    print(f"[debug] trainable params: {trainable:,} / {total:,}")
    print(f"[debug] trainable ratio: {100 * trainable / total:.6f}%")

    for name, p in model.named_parameters():
        if p.requires_grad:
            print("[debug] first trainable param:", name, p.shape, p.dtype)
            break
    else:
        print("[debug] NO TRAINABLE PARAMS")
